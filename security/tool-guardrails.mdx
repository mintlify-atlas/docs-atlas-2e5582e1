...
flowchart TD
    ...
    Decision -->|No| AddContext[Add to context that\ntool is not allowed]

    ToolCall --> Check{Tool result safe?}

    Check -->|Yes| AddTrusted[Add to context and\ncontinue the loop]
    Check -->|No| MarkUntrusted[Mark context as sensitive\nor sanitize via Dual LLM]
