...
The full assigned toolset remains accessible; the model just fetches what it needs rather than receiving everything upfront. [Tool call policies](/security/tool-guardrails) still apply: if the model uses `run_tool` to call `send_email`, Archestra evaluates the `send_email` policy with the same arguments and context as a direct call.
...
Agents can delegate subtasks to other agents. Archestra tracks the full call chain for observability across the entire run. Delegated agents inherit the current [tool guardrails](/security/tool-guardrails) trust state, so downstream policy enforcement does not reset mid-run.
