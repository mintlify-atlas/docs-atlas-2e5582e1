---
title: "Supported LLM Providers in Archestra Platform"
sidebarTitle: "Providers"
description: "Connection details, authentication, and setup steps for every LLM provider supported by the Archestra LLM Proxy, including cloud, self-hosted, and IAM-authenticated options."
---
...
<Accordion title="DeepSeek">
  DeepSeek offers high-performance language models. Use the Model Router to route to DeepSeek with the `deepseek:` model prefix.

  DeepSeek is supported through the Bedrock inference profiles and the Model Router. Configure access through AWS Bedrock or a direct DeepSeek API key via the OpenAI-compatible route.
</Accordion>
